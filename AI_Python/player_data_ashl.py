...
# *****for forward******
df1 = pd.DataFrame(data_frame_forward, columns=["CK", "ST", "DI", "SK", "DF", "PH", "PA", "SC",'Name', 'ProTeamName', 'Age', 'SalaryCap', 'PosC', 'PosLW',	'PosRW', 'PosD'])
...
checkbox_button_group.js_on_event("button_click", CustomJS(code="""
    ...
    for(var column in this.active){
        console.log(labels[this.active[column]]);
        console.log(source.data[labels[this.active[column]]]);
        for(var index in source.data[labels[this.active[column]]]){
            if(source.data['St_Mik_off'][index] == undefined ){
                source.data['St_Mik_off'][index] = source.data[labels[this.active[column]]][index];
            }else{
                source.data['St_Mik_off'][index] = source.data['St_Mik_off'][index]+source.data[labels[this.active[column]]][index];
            }
        };
    }
...
# *****for defence******
df2 = pd.DataFrame(data_frame_defence, columns=["CK", "ST", "DI", "SK", "DF", "PH", "PA", "SC",'Name', 'ProTeamName', 'Age', 'SalaryCap', 'PosC', 'PosLW',	'PosRW', 'PosD'])
...
TOOLTIPS_def = HoverTool(tooltips=[
    ("index", "$index"),
    ("Name", "@Name"),
    ("St_Mik", "@St_Mik_def"),
    ("SalaryCap","@SalaryCap"),
...
checkbox_button_group_def.js_on_event("button_click", CustomJS(code="""
    ...
    for(var column in this.active){
        ...
        console.log(source.data[labels[this.active[column]]]);
        for(var index in source.data[labels[this.active[column]]]){
            if(source.data['St_Mik_def'][index] == undefined ){
                source.data['St_Mik_def'][index] = source.data[labels[this.active[column]]][index];
            }else{
                source.data['St_Mik_def'][index] = source.data['St_Mik_def'][index]+source.data[labels[this.active[column]]][index];
            }
        };
    }
    
    console.log(source.data['St_Mik_def']);
    source.change.emit();
"""))
...
plot_def = figure(width=1500, height=1000, tools=[TOOLTIPS_def], title="Graphique des défenseurs")
